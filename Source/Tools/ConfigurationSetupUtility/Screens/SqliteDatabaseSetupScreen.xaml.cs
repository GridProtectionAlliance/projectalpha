    ...
    public partial class SqliteDatabaseSetupScreen : UserControl, IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            try
            {
                ...
                m_sqliteDatabaseFilePathTextBox.Text = Path.Combine(sqliteDatabaseFilePath, migrate ? "ProjectAlpha" + App.DatabaseVersionSuffix + ".db" : "ProjectAlpha.db");
            }
            catch
            {
                m_sqliteDatabaseFilePathTextBox.Text = migrate ? "ProjectAlpha" + App.DatabaseVersionSuffix + ".db" : "ProjectAlpha.db";
            }
            ...
            if (existing && !migrate)
            {
                serviceConfig = XDocument.Load(FilePath.GetAbsolutePath("ProjectAlpha.exe.config"));
