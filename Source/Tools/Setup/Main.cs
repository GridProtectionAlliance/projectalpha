...
namespace Setup
{
    public partial class Main : Form
    {
        ...
        private void buttonInstall_Click(object sender, EventArgs e)
        {
            ...
            RegistryKey ProjectAlphaInstallKey;

            ProjectAlphaInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            // If key wasn't found, test for 32-bit virtualized location
            if (ProjectAlphaInstallKey == null)
                ProjectAlphaInstallKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\" + ProductCode);

            if (ProjectAlphaInstallKey != null)
            {
                if (MessageBox.Show("An existing version of the ProjectAlpha is installed on this computer. Would you like to remove the existing version?\r\n\r\nCurrent configuration will be preserved.", "Previous Version Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        private bool RunSetup(SetupType type, bool closeOnSuccess)
        {
            ...
            // Install or uninstall ProjectAlpha
            Process ProjectAlphaInstall = new Process();

            ProjectAlphaInstall.StartInfo.FileName = "msiexec.exe";
            ...
            if (type == SetupType.Uninstall)
            {
                ...
                // Uninstall any version of the ProjectAlpha
                ProjectAlphaInstall.StartInfo.Arguments = "/x " + ProductCode + " /qr";
            }
            else
            {
                // Install current version of the ProjectAlpha
                ProjectAlphaInstall.StartInfo.Arguments = "/i Installers\\ProjectAlphaSetup.msi";
            }

            ProjectAlphaInstall.StartInfo.UseShellExecute = false;
            ProjectAlphaInstall.StartInfo.CreateNoWindow = true;
            ProjectAlphaInstall.Start();
            ProjectAlphaInstall.WaitForExit();

            if (ProjectAlphaInstall.ExitCode == 0)
            {
                // Run configuration setup utility post installation of ProjectAlpha, but not for uninstalls
                if (type == SetupType.Install)
                {
                    ...
                    installPath = AddPathSuffix(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\ProjectAlpha", "InstallPath", "").ToString().Trim());
                    targetBitSize = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Grid Protection Alliance\ProjectAlpha", "TargetBitSize", "64bit").ToString().Trim();
                    ...
                    {
                        MessageBox.Show("Setup program was not able to launch the ProjectAlpha Configuration Setup Utility due to an exception. You will need to run this program manually before starting the ProjectAlpha.\r\n\r\nError: " + ex.Message, "Configuration Setup Utility Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void AttemptToStopKeyProcesses()
        {
            ...
            ServiceController ProjectAlphaServiceController = null;

            try
            {
                foreach (ServiceController service in ServiceController.GetServices())
                {
                    if (string.Compare(service.ServiceName, "ProjectAlpha", true) == 0)
                    {
                        ProjectAlphaServiceController = service;
                        break;
                    }
                }
            }
